...
router.get('/list', async (req, res) => {
    ...
    jwt.verify(token, '${process.env.SECRET_KEY}', async (err, decoded) => {
        ...
        try {
            const { country, confirm, startDate, endDate, minCost, maxCost } = req.query;
            ...
            if (endDate) {
                matchStage.endDate = matchStage.endDate || {};
                matchStage.endDate.$lte = new Date(endDate);
            }

            // 총 비용 필터링
            if (minCost || maxCost) {
                matchStage.totalCost = {};
                if (minCost) matchStage.totalCost.$gte = parseInt(minCost);
                if (maxCost) matchStage.totalCost.$lte = parseInt(maxCost);
            }
    ...
    const mailOptions = {
        ...
        html: `
            ...
            <ul>
                ...
                <li>여행기간: ${new Date(bookingData.startDate).toLocaleDateString()} ~ ${new Date(
            bookingData.endDate
