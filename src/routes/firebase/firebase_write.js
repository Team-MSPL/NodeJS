...
//탐방 코스 관련
async function writeReviewOnHiking(mountainName, courseName, review) {
    try {
        const documentPath = mountainName + '/' + courseName;

        // Firestore에서 데이터 읽기
        const oneHikingSnapshot = await database.doc(documentPath).get();
        let reviewList = [];

        //데이터가 있을 경우
        if (
            !(
                oneHikingSnapshot.data().review === null ||
                oneHikingSnapshot.data().review === undefined ||
                oneHikingSnapshot.data().review.length === 0
            )
        ) {
            reviewList = oneHikingSnapshot.data().review;
        }

        //console.log(oneHikingSnapshot.data().review);
        ...
        console.log('탐방 코스 정보 데이터를 읽어오는 중에 오류가 발생했습니다:', error);
        return { status: 'read fail' };
    }
}
...
async function deleteReviewOnHiking(mountainName, courseName, reviewId) {
    try {
        ...
        const reviewList = oneHikingSnapshot.data().review;
...
module.exports.writeReviewOnHiking = writeReviewOnHiking;
module.exports.deleteReviewOnHiking = deleteReviewOnHiking;
